...
/**
 * @title SettlementDeltaForkTest
 * @notice Fork tests exercising EIP-712 order verification and zero-sum delta
 *         accounting through real Aave V3 pool interactions using the Settlement contract.
 *
 *         Uses the same Aave V3 Prime → Core migration pattern as MigrationFork.t.sol
 *         but through Settlement.settleWithFlashLoan, which:
 *           1. Verifies the EIP-712 signature and recovers the signer
 *           2. Runs pre-actions → intent → post-actions → verifyZeroDeltas
 */
contract SettlementDeltaForkTest is Test {
    // Ethereum mainnet
    ...
    uint256 constant USER_BORROW      = 1_000e6;
    uint256 constant WHALE_COLLATERAL = 500 ether;
    uint256 constant WHALE_BORROW     = 500_000e6;
    ...
    function _buildMigration(
        ...
        bytes memory borrowData   = abi.encodePacked(uint8(2), destPool);
        ...
        // Empty settlementData (no swap needed for same-asset migration)
        o.settlementPayload = hex"";
    ...
    function setUp() public {
        ...
        (user, userPk) = makeAddrAndKey("deltaUser");
        whale = makeAddr("deltaWhale");
    ...
    function _setupUserOnPrime() internal {
    ...
    function _pumpPrimeRate() internal {
    ...
    // ═══════════════════════════════════════════════════════════
    //  Test: balanced migration with EIP-712 sig verification
    // ═══════════════════════════════════════════════════════════

    function test_forkDelta_balancedMigration() public {
        ...
        console.log("Prime aWETH balance :", IERC20(aWETH_src).balanceOf(user));
        console.log("Prime USDC debt     :", IERC20(vDebtUSDC_src).balanceOf(user));
        ...
        settlement.settleWithFlashLoan(
            ...
            o.orderData,
            o.executionData,
            hex"" // no fillerCalldata → intent is no-op
        );

        // Verify position migrated
        ...
        console.log("--- Post-migration (sig + delta validated) ---");
    ...
    function test_forkDelta_expiredDeadline_reverts() public {
        if (address(settlement) == address(0)) return;
    ...
    function test_forkDelta_wrongSigner_reverts() public {
        if (address(settlement) == address(0)) return;
    ...
    // ═══════════════════════════════════════════════════════════
    //  Test: unbalanced amounts revert with UnbalancedSettlement
    // ═══════════════════════════════════════════════════════════

    function test_forkDelta_unbalancedBorrow_reverts() public {
        if (address(settlement) == address(0)) return;
        ...
        bytes memory executionData = abi.encodePacked(
            ...
            _action(WETH, type(uint112).max, address(settlement), 3, 0, withdrawData, pr1),
            _action(WETH, 0, user, 0, 0, depositData, pr2),
            _action(USDC, uint112(excessBorrow), address(settlement), 1, 0, borrowData, pr3)
        );
    ...
    function test_forkDelta_invalidProof_reverts() public {
        if (address(settlement) == address(0)) return;
